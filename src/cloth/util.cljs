...
(defn zero-pad
  ...
  ([buffer length] (zero-pad buffer length false))
  ([buffer length right?]
    ((aget eth-util "setLengthLeft") buffer length right?)))
...
(defn solidity-uint [length val]
  (->hex (zero-pad (int->buffer val) length)))
...
(defmethod solidity-format :bytes
  ...
  (let [buffer (Buffer. val)]
    (if-let [size (extract-size type)]
      (->hex (zero-pad buffer size true))
      (let [size (.-length buffer)]
        (str (solidity-uint 32 size)
             (->hex (zero-pad buffer (storage-length size) true)))))))
